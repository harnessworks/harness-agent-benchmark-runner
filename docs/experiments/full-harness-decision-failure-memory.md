...
- A generic prompt-guard mitigation was then added to tell agents to start a
  small scoped repository edit after identifying the relevant implementation
  direction. A follow-up 8-record diagnostic completed 7/8 strict and
...
- A bounded 16-record decision-bearing gate under the strengthened prompt guard
  then completed 16/16 strict and record-consistent with zero no-edit
  watchdogs. The previously recurring `decision-only` replenishment path was
  4/4 strict and record-consistent. This makes a scoped guarded H1 promotion
  attempt defensible again, but it is not itself promotion-scale evidence.
- The subsequent scoped 96-record H1 promotion stopped after 8/96 planned
  records on a `full-harness` replenishment no-edit watchdog. This was a
  startup/no-output no-edit: the Codex log showed startup metadata, the full
  prompt, and repeated rollout state warnings, but no assistant message and no
  repository changes for 240 seconds. Treat this separately from
  post-planning no-edit; promotion-scale operational stability is still
  unresolved.
- A startup-retry H1 pilot then stopped after 1/8 planned records on a
  `decision-only` price-policy no-edit watchdog. The new
  `--retry-startup-no-output-once` gate correctly did not retry it: the agent
  found the accepted price-band decision, announced that it was making the
...
- A two-family Claude adapter-control over price-policy and replenishment then
  completed 4/4 records with zero no-edit watchdogs, zero timeouts, and zero
  file-boundary issues. Strict success was 3/4. `decision-only` passed 2/2
  strict and record-consistent; `full-harness` passed replenishment and missed
...
- A Claude four-arm gate then completed 8/8 records with zero no-edit
  ...
  concept wording.
- The price-policy glossary oracle was then patched to accept equivalent
  `price band`, `price-band`, and `price_band` concept spellings while still
  requiring the `/catalog/price-policy` route. A fresh Claude four-arm gate
  confirmed the price-policy fix: `decision-only` and `full-harness` passed
  strict and record-consistent checks, while `workflow-only` and
